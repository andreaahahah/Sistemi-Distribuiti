...
PROJECT_ID = os.environ.get("GOOGLE_CLOUD_PROJECT", "sistemidistribuiti-butte-dbfb6")
...
def dispatch_nlp_task(article_id: str, plain_text: str) -> None:
    if USE_PUBSUB and publisher:
        try:
            ...
            payload = {"article_id": article_id, "text": plain_text}
    ...
    threading.Thread(target=_run_local, args=(article_id, plain_text), daemon=True).start()
